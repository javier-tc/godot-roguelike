...
func _close_entrance() -> void:
	for entry_position in entrance.get_children():
		tilemap.set_cellv(tilemap.world_to_map(entry_position.position), 8)
		tilemap.set_cellv(tilemap.world_to_map(entry_position.position) + Vector2.DOWN, 2)

func _spawn_enemies() -> void:
	for enemy_position in enemy_positions_container.get_children():
		var random_enemy: String = ENEMY_SCENES.keys()[randi() % ENEMY_SCENES.size()]
		#var enemy: KinematicBody2D = ENEMY_SCENES["FLYING_CREATURE"].instance()
		var enemy: KinematicBody2D = ENEMY_SCENES[random_enemy].instance()
