	...
	for i in num_levels:
		...
		if i == 0:
			...
			for y in corridor_height:
				...
				previous_room_tilemap.set_cellv(exit_tile_pos + Vector2(-1,-y), FLOOR_TILE)
				previous_room_tilemap.set_cellv(exit_tile_pos + Vector2(0,-y), FLOOR_TILE)
